...
public class ServiceValidator {
    ...
    private void validateServiceObject() {
        ...
        List<Node> remoteOrResourceMethodNodes = getRemoteOrResourceMethodNodes(
                objectConstructorExpressionNode.members());
    ...
    private void validateLoadMethodParams(MethodSymbol methodSymbol, String loadMethodName,
        ...
        for (ParameterSymbol symbol : parameterSymbols) {
            ...
            if (!fieldMethodParamSignatures.contains(symbol.signature())) {
                addDiagnostic(CompilationDiagnostic.INVALID_PARAMETER_IN_LOAD_METHOD, methodLocation,
                              symbol.signature(), loadMethodName, expectedFieldName);
            }
        }

        if (!hasDataLoaderParam) {
            addDiagnostic(CompilationDiagnostic.MISSING_DATA_LOADER_PARAMETER, methodLocation, loadMethodName);
        }
    }
