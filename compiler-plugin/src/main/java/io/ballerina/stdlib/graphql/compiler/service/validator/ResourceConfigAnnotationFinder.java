...
public class ResourceConfigAnnotationFinder {
    ...
    private Optional<AnnotationNode> getAnnotationNodeFromModule() {
        ...
        FunctionDefinitionNodeVisitor methodNodeVisitor = new FunctionDefinitionNodeVisitor(this.semanticModel,
                                                                                            this.methodSymbol);
        for (DocumentId documentId : documentIds) {
            Node rootNode = currentModule.document(documentId).syntaxTree().rootNode();
            rootNode.accept(methodNodeVisitor);
            if (methodNodeVisitor.getAnnotationNode().isPresent()) {
                break;
            }
        }
        return methodNodeVisitor.getAnnotationNode();
    }
}