...
public class RecordTypeDefinitionNodeFinder {
    ...
    public Optional<TypeDefinitionNode> find() {
        ...
        RecordTypeDefinitionNodeVisitor recordNodeVisitor = new RecordTypeDefinitionNodeVisitor(this.semanticModel,
                                                                                                this.recordTypeSymbol,
                                                                                                this.recordTypeName);
        for (DocumentId documentId : documentIds) {
            Node rootNode = currentModule.document(documentId).syntaxTree().rootNode();
            rootNode.accept(recordNodeVisitor);
            Optional<TypeDefinitionNode> typeDefinitionNode = recordNodeVisitor.getRecordTypeDefinitionNode();
