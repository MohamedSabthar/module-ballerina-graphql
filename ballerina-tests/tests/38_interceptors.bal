...
isolated function testInterceptorsWithSubscriptionReturningRecord() returns error? {
    ...
    json expectedPayload =  {'type: WS_NEXT, id:"1", payload: {data: {books: {name: "Crime and Punishment", author: "Athur Conan Doyle"}}}};
    check validateWebSocketResponse(wsClient, expectedPayload);
    expectedPayload =  {'type: WS_NEXT, id:"1", payload: {data: {books: {name: "A Game of Thrones", author: "Athur Conan Doyle"}}}};
    check validateWebSocketResponse(wsClient, expectedPayload);
}
...
isolated function testInterceptorsWithSubscriptionAndFragments() returns error? {
    ...
    check validateConnectionAckMessage(wsClient);
    check sendSubscriptionMessage(wsClient, document);
    json expectedPayload = {'type: WS_NEXT, id:"1", payload: {data: {students: {id: 1, name: "Harry Potter"}}}};
    check validateWebSocketResponse(wsClient, expectedPayload);
    expectedPayload =  {'type: WS_NEXT, id:"1", payload: {data: {students: {id: 2, name: "Harry Potter"}}}};
    check validateWebSocketResponse(wsClient, expectedPayload);
}
...
isolated function testInterceptorsWithSubscribersRunSimultaniously1() returns error? {
    ...
    check sendConnectionInitMessage(wsClient3);
    check validateConnectionAckMessage(wsClient3);
    ...
    foreach int i in 1 ..< 4 {
        json expectedPayload = {data: {messages: i * 5 - 5}};
        check validateWebSocketResponse(wsClient3, {'type: WS_NEXT, id:"3", payload:expectedPayload});
    }
    check wait A;
    check wait B;
}
...
isolated function testInterceptorsWithSubscribersRunSimultaniously2() returns error? {
    ...
    worker B returns error? {
        check sendSubscriptionMessage(wsClient2, document, "2");
        json expectedPayload = {'type: WS_NEXT, id:"2", payload:{ 
            data: {
                multipleValues: {
                    id: 100,
                    name: "Jesse Pinkman"
        ...
        expectedPayload = {'type: WS_NEXT, id:"2", payload:{ 
            data: {
                multipleValues: {
                    name: "Walter White",
                    subject: "Physics"
