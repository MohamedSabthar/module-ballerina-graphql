...
class ResponseGenerator {
    ...
    returns anydata {
        if !self.isMutationOperation {
            return self.getResultFromArrayParallely(parentValue, parentNode, path);
        }
        int i = 0;
        anydata[] result = [];
    ...
    isolated function getResultFromArrayParallely((any|error)[] parentValue, parser:FieldNode parentNode, (string|int)[] path)
    returns anydata {
        int i = 0;
        anydata[] result = [];
    ...
    returns anydata {
        if !self.isMutationOperation {
            return self.getResultFromTableParallely(parentValue, parentNode, path);
        }
        anydata[] result = [];
    ...
    isolated function getResultFromTableParallely(table<map<any>> parentValue, parser:FieldNode parentNode,
