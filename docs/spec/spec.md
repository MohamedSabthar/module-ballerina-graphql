...
11. [Experimental Features](#11-experimental-features)
    * 11.1 [DataLoader](#111-dataloader)
        ...
        * 11.1.2. [Engaging DataLoaders](#1112-engaging-dataloaders)
            ...
            * 11.1.2.3 [Define the Corresponding `prefetch` Method](#11123-define-the-corresponding-prefetch-method)
...
The `prefetchMethodName` field is used override the default prefetch method name. To know more about the prefetch method name, refer to the [Define the Corresponding `prefetch` Method](#11123-define-the-corresponding-prefetch-method) section.
...
context.registerDataLoader("author", new dataloader:DefaultDataLoader(authorBatchFunction));
```
...
dataloader:DataLoader authorLoader = context.getDataLoader("author");
```
...
##### 11.1.2.2 Register DataLoaders to Context via ContextInit Function
...
##### 11.1.2.3 Define the Corresponding `prefetch` Method
...
In the given example, both the `books` resource function and the `loadBooks` resource function receive the `map<dataloader:DataLoader>` parameter, which grants access to the `DataLoader` objects created by the GraphQL Engine. By using the `ctx.getDataLoader("bookLoader")` syntax, the specific `DataLoader` object associated with the unique identifier "bookLoader" can be obtained and assigned to the `bookLoader` variable.
